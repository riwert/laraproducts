    ...
    @if (isset($product) && $product->prices->count())
        @foreach($product->prices as $price)
            <li class="d-flex justify-content-between mb-2">
                ...
                <input class="form-control mx-1" type="text" name="prices[{{ $loop->iteration }}][name]" value="{{ $price->name or old('prices')[$loop->iteration]['name'] }}" placeholder="{{ __('Nazwa ceny') }}" required="required">
                <input class="form-control mx-1" type="text" name="prices[{{ $loop->iteration }}][value]" value="{{ $price->value or old('prices')[$loop->iteration]['value'] }}" placeholder="{{ __('Wartość ceny') }}" required="required">
                <input class="form-control mx-1" type="text" name="prices[{{ $loop->iteration }}][unit]" value="{{ $price->unit or old('prices')[$loop->iteration]['unit'] }}" placeholder="{{ __('Jednostka ceny') }}" required="required">
